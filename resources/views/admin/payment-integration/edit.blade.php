...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.payment-integration.update') }}" class="dash-card max-w-2xl space-y-5">
        @csrf
        @method('PUT')
    ...
    {{-- PayPal — separate form, Buyahref UPI untouched --}}
    <div class="mb-5 mt-10" id="paypal">
    ...
    @if ($paypal['enabled'] && ! $paypalReady)
        <div class="mb-5 max-w-2xl rounded-xl border border-warning/30 bg-warning/10 px-4 py-3 text-sm text-warning">
    ...
    <form method="POST" action="{{ route('admin.payment-integration.paypal') }}" class="dash-card max-w-2xl space-y-5">
        @csrf
        @method('PUT')
    ...
    <form method="POST" action="{{ route('admin.payment-integration.paypal.test') }}" class="mt-3">
        @csrf
